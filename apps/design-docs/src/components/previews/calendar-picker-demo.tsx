  ...
  return (
    <div className="gap-6 max-w-sm p-4 py-12 mx-auto flex min-h-[300px] w-full flex-col items-center justify-center">
      <div className="gap-2 flex flex-col">
        ...
        <CalendarPicker value={range} onChange={setRange} presets={presets} allowClear />
      </div>

      <div className="gap-2 flex flex-col">
        <span className="text-sm font-medium">Compact Layout</span>
        <CalendarPicker value={range} onChange={setRange} presets={presets} compact allowClear />
      </div>
    </div>
  );
}
