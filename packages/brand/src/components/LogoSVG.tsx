...
export function LogomarkSVG({
  ...
  return (
    <svg
      ...
      height={height}
      className={className}
      aria-label={ariaLabel ?? (ariaHidden ? undefined : "Nebutra")}
      aria-hidden={ariaHidden}
      role={ariaHidden ? undefined : "img"}
      fill="currentColor"
    >
      <path
        d="M518.13,184.13c-12.15-21.05-29.76-37.95-50.42-49.37c0.53-23.73-5.29-47.58-17.52-68.75l-0.14-0.25
...
export function LogoEnSVG({
  ...
  // Logomark is square (viewBox 535.71 x 500), rendered at ~height h
  // Wordmark aspect ratio: 544.21 / 103.74 ≈ 5.246
  // We scale so the wordmark height matches the logomark height
  // Total width = logomarkW + gap + wordmarkW

  const logomarkAspect = 535.71 / 500; // ≈ 1.0714
  const wordmarkAspect = 544.21 / 103.74; // ≈ 5.246

  // Derive heights from total width (iteratively balanced)
  // Treat height as the logomark height, then compute widths
  ...
  return (
    ...
    >
      ...
      {/* Wordmark — offset by logomarkW + gap */}
      <g
